  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed z-[91] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg max-h-[85vh] flex flex-col bg-surface-card border border-border-default rounded-2xl shadow-modal"
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-7">
          ...
          <Step n={1} title="Install Obsidian Web Clipper">
            ...
            <div className="flex gap-2 mt-3">
              <a
                ...
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 bg-surface-raised hover:bg-surface-overlay border border-border-default rounded-lg text-text-primary transition-colors"
              >
            ...
            <div className="flex flex-col gap-2 mt-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="text-xs px-3 py-2 bg-surface-raised hover:bg-surface-overlay border border-border-default rounded-lg text-text-secondary transition-colors"
                >
              ...
              {showTemplate && (
                <div className="relative">
                  <pre className="text-2xs font-mono text-text-secondary bg-surface-raised/80 border border-border-default rounded-lg p-3 overflow-x-auto leading-relaxed max-h-48 overflow-y-auto">
                    {templateContent ?? 'Loading…'}
                  </pre>
                  <button
                    onClick={() => void copyTemplate()}
                    className="absolute top-2 right-2 text-2xs px-2 py-1 bg-surface-overlay hover:bg-surface-overlay rounded transition-colors"
                  >
                    {copied ? 'Copied ✓' : 'Copy'}
                  </button>
                </div>
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 bg-surface-overlay hover:bg-surface-overlay border border-border-strong rounded-lg text-text-primary font-medium transition-colors w-fit"
              >
          ...
          <Step n={3} title="Import templates into Web Clipper">
            ...
            <ol className="text-xs text-text-secondary leading-relaxed space-y-2 list-decimal list-inside">
              <li>Click the Web Clipper icon in your browser toolbar</li>
              <li>
                Open <strong className="text-text-primary">Settings</strong> (gear icon, bottom-left)
              </li>
              <li>
                Go to <strong className="text-text-primary">Templates</strong> → click{' '}
                <strong className="text-text-primary">Import</strong> → select the downloaded{' '}
                <code className="text-indigo-300 bg-indigo-950/40 px-1 rounded text-2xs">.md</code>{' '}
                files — import both
              </li>
              <li>
                The <strong className="text-text-primary">Job listing</strong> template auto-triggers on
                job board URLs (Greenhouse, Ashby, LinkedIn Jobs, and more)
              </li>
              <li>
                The <strong className="text-text-primary">LinkedIn contact</strong> template triggers on{' '}
          ...
          <Step n={4} title="Set the save folder in Web Clipper">
            <p className="text-xs text-text-secondary leading-relaxed">
              ...
              <strong className="text-text-primary">Default save location</strong> to your Jobs folder —
              use the same path you&apos;ll configure in ResumeLoop.
            </p>
            <div className="mt-3 bg-surface-raised/60 border border-border-default rounded-lg px-3 py-2.5 space-y-1">
              <p className="text-2xs text-text-secondary font-mono">~/Documents/Jobs</p>
              <p className="text-2xs text-text-secondary font-mono">~/Obsidian/JobSearch/Jobs</p>
            </div>
          ...
          <Step n={5} title="Clip a job, then scan">
            ...
            <p className="text-xs text-text-secondary leading-relaxed mt-2">
              In ResumeLoop, set <strong className="text-text-primary">Settings → Jobs Folder</strong> to
              that same path, then click <strong className="text-text-primary">Scan</strong> on the Jobs
        ...
        {/* Footer */}
        <div className="shrink-0 border-t border-border-subtle px-6 py-3 flex justify-end bg-surface-card rounded-b-2xl">
          <button
            onClick={onClose}
